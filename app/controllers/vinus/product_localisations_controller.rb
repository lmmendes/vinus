...
module Vinus
  class ProductLocalisationsController < ApplicationController
    ...
    before_filter { @product = Vinus::Product.find(params[:product_id]) }
    ...
    def create
      if I18n.available_locales.include? safe_params[:locale].to_sym
        ...
        if @product.update(safe_params)
          ...
          redirect_to [@product, :localisations], flash: { notice: t('vinus.localisations.localisation_created') }
        else
          render action: 'form'
        end
      else
    ...
    def update
      if @localisation.update(safe_params)
        redirect_to [@product, :localisations], flash: { notice: t('vinus.localisations.localisation_updated') }
      else
        render action: 'form'
      end
    end
    ...
    def destroy
      ...
      redirect_to [@product, :localisations], flash: { notice: t('vinus.localisations.localisation_destroyed') }
    end
