...
module Vinus
  class ProductCategoryLocalisationsController < ApplicationController
    ...
    before_filter { @product_category = Vinus::ProductCategory.find(params[:product_category_id]) }
    ...
    def create
      if I18n.available_locales.include? safe_params[:locale].to_sym
        ...
        if @product_category.update(safe_params)
          ...
          redirect_to [@product_category, :localisations], flash: { notice: t('vinus.localisations.localisation_created') }
        else
          render action: 'edit'
        end
      else
    ...
    def update
      if @localisation.update(safe_params)
        redirect_to [@product_category, :localisations], notice: t('vinus.localisations.localisation_updated')
      else
        render action: 'form'
      end
    end
    ...
    def destroy
      ...
      redirect_to [@product_category, :localisations], notice: t('vinus.localisations.localisation_destroyed')
    end
