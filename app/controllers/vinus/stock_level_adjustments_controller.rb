module Vinus
  class StockLevelAdjustmentsController < ApplicationController
    SUITABLE_OBJECTS = ['Vinus::Product'].freeze
    before_filter do
      fail Vinus::Error, t('vinus.stock_level_adjustments.invalid_item_type', suitable_objects:  SUITABLE_OBJECTS.to_sentence) unless SUITABLE_OBJECTS.include?(params[:item_type])
    ...
    def create
      ...
      if @new_sla.save
        if request.xhr?
          ...
          redirect_to stock_level_adjustments_path(item_id: params[:item_id], item_type: params[:item_type]), notice: t('vinus.stock_level_adjustments.create_notice')
        end
      else
        if request.xhr?
