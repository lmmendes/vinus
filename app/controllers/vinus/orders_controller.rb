module Vinus
  class OrdersController < Vinus::ApplicationController
    before_filter { @active_nav = :orders }
    before_filter { params[:id] && @order = Vinus::Order.find(params[:id]) }

    def index
      @query = Vinus::Order.ordered.received.includes(order_items: :ordered_item).page(params[:page]).search(params[:q])
      @orders = @query.result
    end

    def new
      @order = Vinus::Order.new
      @order.order_items.build(ordered_item_type: 'Vinus::Product')
    end

    def create
      Vinus::Order.transaction do
        @order = Vinus::Order.new(safe_params)
        @order.status = 'confirming'

        if safe_params[:customer_id]
          @customer = Vinus::Customer.find safe_params[:customer_id]
        ...
        if !request.xhr? && @order.save
          @order.confirm!
          redirect_to @order, flash: { notice: t('vinus.orders.create_notice') }
        else
          @order.order_items.build(ordered_item_type: 'Vinus::Product')
          render action: 'new'
        end
      end
    rescue Vinus::Errors::InsufficientStockToFulfil => e
      flash.now[:alert] = t('vinus.orders.insufficient_stock_order', out_of_stock_items: e.out_of_stock_items.map { |t| t.ordered_item.full_name }.to_sentence)
      render action: 'new'
    end
    ...
    def update
      ...
      if !request.xhr? && @order.update_attributes(safe_params)
        redirect_to @order, flash: { notice: t('vinus.orders.update_notice') }
      else
        render action: 'edit'
      end
    end
    ...
    def accept
      ...
      redirect_to @order, flash: { notice: t('vinus.orders.accept_notice') }
    rescue Vinus::Errors::PaymentDeclined => e
      redirect_to @order, flash: { alert: e.message }
    end

    def reject
      @order.reject!(current_user)
      redirect_to @order, flash: { notice: t('vinus.orders.reject_notice') }
    rescue Vinus::Errors::PaymentDeclined => e
      redirect_to @order, flash: { alert: e.message }
    end
    ...
    def ship
      ...
      redirect_to @order, flash: { notice: t('vinus.orders.ship_notice') }
    end

    def despatch_note
      render layout: 'vinus/printable'
    end
