  ...
  class Order < ActiveRecord::Base
    ...
    def confirm!
      ...
      unless no_stock_of.empty?
        fail Vinus::Errors::InsufficientStockToFulfil, order: self, out_of_stock_items: no_stock_of
      end
    ...
    #
    # @param user [Vinus::User] the user who carried out this action
    def accept!(user = nil)
    ...
    #
    # @param user [Vinus::User] the user who carried out the action
    def reject!(user = nil)
    ...
    def deliver_accepted_order_email
      Vinus::OrderMailer.accepted(self).deliver
    end

    def deliver_rejected_order_email
      Vinus::OrderMailer.rejected(self).deliver
    end

    def deliver_received_order_email
      Vinus::OrderMailer.received(self).deliver
    end
  end
end
