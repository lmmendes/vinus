module Vinus
  class Order < ActiveRecord::Base
    ...
    #
    # @return [Vinus::DeliveryService]
    belongs_to :delivery_service, class_name: 'Vinus::DeliveryService'

    # The country where this order is being delivered to (if one has been provided)
    #
    # @return [Vinus::Country]
    belongs_to :delivery_country, class_name: 'Vinus::Country', foreign_key: 'delivery_country_id'

    # The user who marked the order has shipped
    #
    # @return [Vinus::User]
    belongs_to :shipper, class_name: 'Vinus::User', foreign_key: 'shipped_by'
    ...
    before_confirmation do
      ...
      if delivery_required? && !valid_delivery_service?
        fail Vinus::Errors::InappropriateDeliveryService, order: self
      end
    ...
    #
    # @return [Array] an array of Vinus::DeliveryService objects
    ...
    #
    # @return [Array] an array of Vinus:DeliveryServicePrice objects
    def delivery_service_prices
      if delivery_required?
        prices = Vinus::DeliveryServicePrice.joins(:delivery_service).where(vinus_delivery_services: { active: true }).order(:price).for_weight(total_weight)
    ...
    def ship!(consignment_number, user = nil)
      run_callbacks :ship do
        ...
        save!
        Vinus::OrderMailer.shipped(self).deliver
      end
    end
  end
end
