module Vinus
  class Order < ActiveRecord::Base
    ...
    self.table_name = 'vinus_orders'

    # Orders can have properties
    key_value_store :properties

    # Require dependencies
    require_dependency 'vinus/order/states'
    require_dependency 'vinus/order/actions'
    require_dependency 'vinus/order/billing'
    require_dependency 'vinus/order/delivery'
    require_dependency 'vinus/order/packages'

    # All items which make up this order
    has_many :order_items, dependent: :destroy, class_name: 'Vinus::OrderItem', inverse_of: :order
    ...
    has_many :products, through: :order_items, class_name: 'Vinus::Product', source: :ordered_item, source_type: 'Vinus::Product'

    # The order can belong to a customer
    belongs_to :customer, class_name: 'Vinus::Customer'

    has_many :addresses, through: :customers, class_name: 'Vinus::Address'

    # List of unique vendors (via products association)
    #
    # @return [ActiveRecord::Relation] of unique of Vinus::Vendor objects
    has_many :vendors, -> { uniq }, through: :products, class_name: 'Vinus::Vendor'

    has_many :packages, class_name: 'Vinus::Package'

    # TODO Nees to be done
    # has_many :packages_for_vendor, { where(vendor: vendor) }

    scope :for_vendor, -> (vendor) { joins(:vendors).where('vinus_products.vendor_id' => vendor) }
