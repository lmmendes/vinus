...
module Vinus
  class Product < ActiveRecord::Base
    self.table_name = 'vinus_products'

    # Add dependencies for products
    require_dependency 'vinus/product/product_attributes'
    require_dependency 'vinus/product/variants'

    # Attachments for this product
    has_many :attachments, as: :parent, dependent: :destroy, autosave: true, class_name: 'Vinus::Attachment'

    # The product's categorizations
    #
    # @return [Vinus::ProductCategorization]
    has_many :product_categorizations, dependent: :destroy, class_name: 'Vinus::ProductCategorization', inverse_of: :product
    # The product's categories
    #
    # @return [Vinus::ProductCategory]
    has_many :product_categories, class_name: 'Vinus::ProductCategory', through: :product_categorizations

    # The product's tax rate
    #
    # @return [Vinus::TaxRate]
    belongs_to :tax_rate, class_name: 'Vinus::TaxRate'

    # Ordered items which are associated with this product
    has_many :order_items, dependent: :restrict_with_exception, class_name: 'Vinus::OrderItem', as: :ordered_item

    # Orders which have ordered this product
    has_many :orders, through: :order_items, class_name: 'Vinus::Order'

    # Stock level adjustments for this product
    has_many :stock_level_adjustments, dependent: :destroy, class_name: 'Vinus::StockLevelAdjustment', as: :item

    # The vendor which this product should be linked to
    #
    # @return [Vinus::Vendor]
    belongs_to :vendor, class_name: 'Vinus::Vendor'

    has_one :merchant, class_name: 'Vinus::Merchant', through: :vendor
    ...
    # alias_method :price_with_tax, :full_price
    ...
    # For example:
    #
    #   Vinus::Product.active.with_attribute('Manufacturer', 'Apple').with_attribute('Model', ['Macbook', 'iPhone'])
    #
    # @return [Enumerable]
    def self.with_attributes(key, values)
      product_ids = Vinus::ProductAttribute.searchable.where(key: key, value: values).pluck(:product_id).uniq
    ...
    # Example:
    #
    #   Vinus:Product.import("path/to/file.csv")
    def self.import(file)
      ...
      (2..spreadsheet.last_row).each do |i|
        ...
        if product = where(name: row['name']).take
          ...
          qty = row['qty'].to_i
          if qty > 0
            product.stock_level_adjustments.create!(description: I18n.t('vinus.import'), adjustment: qty)
          end
        else
          product = new
          ...
          product.product_categories << begin
            Vinus::ProductCategory.find_or_initialize_by(name: row['category_name'])
          end
    ...
    def self.open_spreadsheet(file)
      ...
      else fail I18n.t('vinus.imports.errors.unknown_format', filename: File.original_filename)
      end
    end
