module Vinus
  class OrderItem < ActiveRecord::Base
    self.table_name = 'vinus_order_items'

    # The associated order
    #
    # @return [Vinus::Order]
    belongs_to :order, class_name: 'Vinus::Order', touch: true, inverse_of: :order_items
    ...
    has_many :stock_level_adjustments, as: :parent, dependent: :nullify, class_name: 'Vinus::StockLevelAdjustment'
    ...
    # exist already.
    #
    # @param ordered_item [Object] an object which implements the Vinus::OrderableItem protocol
    # @param quantity [Fixnum] the number of items to order
    # @return [Vinus::OrderItem]
    ...
    def increase!(amount = 1)
      transaction do
        ...
        unless in_stock?
          fail Vinus::Errors::NotEnoughStock, ordered_item: ordered_item, requested_stock: self.quantity
        end
        save!
        order.remove_delivery_service_if_invalid
      end
