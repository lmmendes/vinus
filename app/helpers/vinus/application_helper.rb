module Vinus
  module ApplicationHelper
    ...
    def status_tag(status)
      content_tag :span, t("vinus.orders.statuses.#{status}"), class: "status-tag #{status}"
    end
    ...
    def settings_label(field)
      "<label for='settings_#{field}'>#{t("vinus.settings.labels.#{field}")}</label>".html_safe
    end

    def settings_field(field, options = {})
      default = I18n.t('vinus.settings.defaults')[field.to_sym]
      value = (params[:settings] && params[:settings][field]) || Vinus.settings[field.to_s]
      type = I18n.t('vinus.settings.types')[field.to_sym] || 'string'
      case type
      when 'boolean'
        ''.tap do |s|
          ...
          s << label_tag("settings_#{field}_true", t("vinus.settings.options.#{field}.affirmative", default: 'Yes'))
          s << radio_button_tag("settings[#{field}]", 'false', value == false, id: "settings_#{field}_false")
          s << label_tag("settings_#{field}_false", t("vinus.settings.options.#{field}.negative", default: 'No'))
          s << '</div>'
        end.html_safe
      else
